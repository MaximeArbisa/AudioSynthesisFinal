function [SIGNAL, SIGNALS, Dm] = PTA_TransientOn(x, Fs, nbInstru, display)
% Pitch Time Amplitude (PTA) algorithm developped by J. Pätynen, where
% transient detection and processing was added
%
% Inputs:
%       - x: original signal
...
for h = 1:nbInstru
    ...
    pitch = normrnd(2, 0); % normal distribution - 0.5% pitch modification
    ...
    for k = 2:Nt
        ...
        if ismember(k, transients) % If transient, reinitialize the phase
            Y(:,k) = X(:,k);
            phase = angle(X(:,k));            
        else % Compute new phase - normal technique

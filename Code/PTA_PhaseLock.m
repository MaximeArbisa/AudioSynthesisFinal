function [SIGNAL, SIGNALS, Dm] = PTA_PhaseLock(x, Fs, nbInstru, phaselock, display)
% Pitch Time Amplitude (PTA) algorithm developped by J. Pätynen, with
% identity phase locking technique, within and across peak channels,
% respectively achieved with 'intra' and 'inter' phaselock option.
%
% Inputs:
%       - x: original signal
%       - Fs: sampling frequency
%       - nbInstru: number of instruments
%       - phaselock: phaselocking technique - 'intra' for phase unwrapping
%       within same peak channels. 'inter' for phase unwrapping accross
%       close peak channels.
...
if strcmp(phaselock, 'intra') % Phase locking intra peaks
    for k=2:Nt  % Loop on timeframes
        ...
        if isempty(pks) % If no peaks
            pks = (1:Nfft); % Treat all channels
            zones = []; % No zones
        end
        
        
        % Time stretching, using phase unwrapping method
        diff_phase = (angle(X(pks,k)) - former_phase(pks)) - puls(pks);
        ...
        % Save informations for synthesis
        AnalysisInfos(k-1).pks = pks;
        AnalysisInfos(k-1).zones = zones;
        AnalysisInfos(k-1).associatedPks = pks; % For inter peaks computation
        % pks = associatedPks for intra peaks computation
        AnalysisInfos(k-1).instantFreq = instantFreq;
    end
    
elseif strcmp(phaselock, 'inter') % Phaselocking inter peaks
    ...
    clc;
    error('Phase lock option ''phaselock'' must equal ''inter'' or ''intra''');
end
disp('STFT computed');
...
for h = 1:nbInstru
    ...
    for k = 2:Nt
        ...
        Y(pks,k) = abs(X(pks,k)).*exp(1i*phase(pks));
        
        % Update zones
        zones = AnalysisInfos(k-1).zones; % Zones
        teta = phase(pks) - angle(X(pks,k)); % Teta and phasor Z
        Z = exp(1i.*teta);
        for l = 1:length(zones)-1
            Y(zones(l):zones(l+1),k) = Z(l).*X(zones(l):zones(l+1),k);
        end
        phase = angle(Y(:,k)); % Update phase for the next frame
